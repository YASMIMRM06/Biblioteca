                ...
                <form method="POST" action="{{ route('usuarios.update', $usuario->id) }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input id="name" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="text" name="name" value="{{ old('name', $usuario->name) }}" required autofocus />
                            @error('name')
                        ...
                        <div>
                            ...
                            <input id="email" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="email" name="email" value="{{ old('email', $usuario->email) }}" required />
                            @error('email')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="password" class="block font-medium text-sm text-gray-700">{{ __('Nova Senha (deixe em branco para não alterar)') }}</label>
                        ...
                        <div>
                            <label for="password_confirmation" class="block font-medium text-sm text-gray-700">{{ __('Confirmar Nova Senha') }}</label>
                        ...
                        <div>
                            ...
                            <input id="telefone" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="text" name="telefone" value="{{ old('telefone', $usuario->telefone) }}" />
                            @error('telefone')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select id="tipo" name="tipo" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach ($tiposUsuario as $tipo)
                                    <option value="{{ $tipo }}" {{ old('tipo', $usuario->tipo) == $tipo ? 'selected' : '' }}>{{ ucfirst($tipo) }}</option>
                                @endforeach
                            </select>
                            @error('tipo')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
