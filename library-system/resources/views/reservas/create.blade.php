                ...
                <form method="POST" action="{{ route('reservas.store') }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input id="data_reserva" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" type="datetime-local" name="data_reserva" value="{{ old('data_reserva', now()->format('Y-m-d\TH:i')) }}" required />
                            @error('data_reserva')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
